...
public class AnimationReader {
  ...
  private static <Doc> void readMotion(Scanner s, AnimationBuilder<Doc> builder) {
    String[] fieldNames = new String[] { "initial time", "initial x-coordinate",
        ...
        "final y-coordinate", "final width", "final height", "final red value", "final green value",
        "final blue value", };
    int[] vals = new int[16];
    String name;
    if (s.hasNext()) {
    ...
    builder.addMotion(name, vals[0], vals[1], vals[2], vals[3], vals[4], vals[5], vals[6], vals[7],
  ...
  private static int getInt(Scanner s, String label, String fieldName) {
    if (s.hasNextInt()) {
      ...
      throw new IllegalStateException(String
          .format("%s: expected integer for %s, but no more input available", label, fieldName));
    }
  }
